...
function [freqs_Hz]=truss_model_func_5parm(pos1, pos2, k_b , G_b, G_v)
%% A wrapped version of moving mass truss structure
%% Parameters:
...
%k_b = 2e7; % base restrictions  **Used as input

%% material property
E = 6.9E10; % Pa
poisson = 0.33; % Poisson's ratio
G = E/2/(1+poisson); % shear modulus Pa
% G_v = G;% Pa  **Used as input
% G_b = G; %shear modulus for bottom horizontal beam **Used as input
...
j = find(remainlist == pos1); % input position1
dof = (j-1)*ndof0+1:j*ndof0;
...
j = find(remainlist == pos2); % input position2
dof = (j-1)*ndof0+1:j*ndof0;
newMr(dof,dof) = newMr(dof,dof) + diag([2.03 2.03 2.03 2*2.03*0.0117^2 0 2*2.03*0.0117^2]);
